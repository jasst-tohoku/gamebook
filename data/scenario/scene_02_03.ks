...
_　プロジェクト後期に出てくる試作機や本番機も評価環境と同じという訳には行かないだろう。共通化を意識しておかないと管理しにくくなりそうだ。[l][r]
[r]
...
_　ソフトウェア開発に必要なデバイスのインターフェース仕様はプロジェクト初期に決めるそうで、現時点でもいくつかのドキュメントは揃っている。[l][r]
_　しかし、デバイス内部の細かな仕様はプロジェクト中期までかかりそうだ。[l][r]
...
_　プロジェクト後期に入ったある日に、部長に呼び止められた。評価機で実装できる部分は全部実装し終わり、今は試作機の到着待ちの状況である。[l][r]
...
_　シミュレーターを作るにはハードウェアの仕様を把握する必要があるが、プロジェクト初期の現時点ではハードウェアの使用は不明確だ。[l][r]
